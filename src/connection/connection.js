...
class Connection extends EventEmitter {
  ...
  onErrResponse(bufferReader) {
    const errCode =
      bufferReader.getRemainingBytesCount() > 0
  ...
  onChannelInfoResponse(bufferReader) {
    ...
    if (remainingBytesLength === 16) {
      ...
      console.log(
        `ChannelInfo has unexpected key length: ${remainingBytesLength}`,
      );
    }
  }
  ...
  setRadioParams(radioFreq, radioBw, radioSf, radioCr) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // set tx power
        await this.sendCommandSetRadioParams(
          radioFreq,
          radioBw,
          radioSf,
          radioCr,
  ...
  async findContactByPublicKeyPrefix(pubKeyPrefix) {
    ...
    return contacts.find((contact) => {
      const contactPubKeyPrefix = contact.publicKey.subarray(
        0,
        pubKeyPrefix.length,
      );
  ...
  sendChannelTextMessage(channelIdx, text) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // send message
        await this.sendCommandSendChannelTxtMsg(
          txtType,
          channelIdx,
          senderTimestamp,
          text,
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const onLoginSuccess = (response) => {
          ...
          if (
            !BufferUtils.areBuffersEqual(publicKeyPrefix, response.pubKeyPrefix)
          ) {
            console.log(
              'onLoginSuccess is not for this login request, ignoring...',
            );
            return;
          }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const onBinaryResponsePush = (response) => {
          ...
          if (tag !== response.tag) {
            console.log(
              'onBinaryResponse is not for this request tag, ignoring...',
            );
            return;
          }
        ...
        // send binary request
        await this.sendCommandSendBinaryReq(
          contactPublicKey,
          requestCodeAndParams,
        );
      } catch (e) {
  ...
  async getNeighbours(
    ...
    // send binary request
    const responseData = await this.sendBinaryRequest(
      publicKey,
